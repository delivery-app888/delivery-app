...
スクリプト・ツール・ハーネス・検証ジグなどの実行資産は、**登記と導線がなければ忘れ去られて再発明される**。以下を守ること：

1. **作る前**: `docs/tools-registry.md`（ツール登記所）を見る。同じ用途のツールが既にあれば再発明せず使い、最終使用日を更新する
2. **作ったら**: 使い捨てでない限り、登記所への登記＋呼び出し導線の配線（workflow-orchestration への組込み / npm scripts / スラッシュコマンド / フックのどれか1本）までを**完成の定義**とする。「動いた」だけでは完成と報告しない
3. **使い捨ての判定**: 「3ヶ月後にこの作業をもう一度やるとき、これを見つけられなければ作り直すか？」→ Yes なら登記、No なら使用後に削除（残すのに登記しない、が最悪）

---

## 他の AI への引き渡しは「専用プロンプトの同時生成」で行う

Codex・ChatGPT・NotebookLM・Gemini など**別の AI に作業を渡す場面では、ユーザーがそのまま貼り付けられる専用プロンプトを必ず同時に生成し、チャットにコードブロックで提示する**（依頼の言及だけで終えない。貼り付け作業の設計までが引き渡しの完成）。

- **5部構成**: ①役割と前提（確定済み事項と変更禁止の境界。疑義は実行せず報告、の逃げ道つき）②読むもの（相手はコンテキストを持たない前提でファイル列挙）③固有の絶対条件（この案件で事故りやすい点）④出力（形式・保存先）⑤完了報告事項（検収可能な形）
- **成熟の階段**: 1回目=チャットで即席生成 → 同型2回目=穴埋めテンプレとして手順書に凍結+tools-registry 登記 → 定型=スクリプト化
- 一般論と実装例は `~/Dev/my-context/ai-workflows.md` の該当節を参照（参考実装: my-channel-project docs/phase-i-runbook.md §3 / scripts/factcheck-export.py）

<!-- my-context-setup: v5 (2026-07-08) -->
